...
/// fee refunds.
/// @dev Commented the lines in charge of validating the proof for testing purposes
library LibBridgeProcessForTest {
  ...
  function processMessage(
    ...
    // Commented only the lines in charge of validating the proof
    // if (
    //   checkProof &&
    ...
    if (ethVault != address(0)) {
      EtherVault(payable(ethVault)).releaseEther(address(this), message.value + message.fee);
    }
    ...
    if (message.to == address(this) || message.to == address(0)) {
      ...
      // use remaining gas
      uint256 gasLimit = msg.sender == message.user ? gasleft() : message.gasLimit;
    ...
    address refundTo = message.refundTo == address(0) ? message.user : message.refundTo;

...
library LibBridgeRecall {
  ...
  function recallMessage(
    ...
    if (checkProof && !LibBridgeStatus.isMessageFailed(resolver, msgHash, message.destChainId, proof)) {
    ...
    if (ethVault != address(0)) {
      EtherVault(payable(ethVault)).releaseEther(address(this), message.value);
    }

    // Execute the recall logic based on the contract's support for the
    // IRecallableMessageSender interface
    if (message.from.supportsInterface(type(IRecallableMessageSender).interfaceId)) {
      IRecallableMessageSender(message.from).onMessageRecalled{value: message.value}(message);
    } else {

...
import {LibSecureMerkleTrie} from "./LibSecureMerkleTrie.sol";
...
library LibTrieProof {
  ...
  function verifyWithFullMerkleProof(
    ...
    (bool exists, bytes memory rlpAccount) = LibSecureMerkleTrie.get(abi.encodePacked(addr), accountProof, stateRoot);
    ...
    bytes32 storageRoot = LibRLPReader.readBytes32(accountState[ACCOUNT_FIELD_INDEX_STORAGE_HASH]);

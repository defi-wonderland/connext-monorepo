...
contract MockArbitrumRollup {
  ...
  function setNode(bytes32 _confirmData, uint64 _stakerCount, uint64 _childStakerCount) public {
    Node memory node;
...
contract MockArbitrumOutbox {
  ...
  function calculateMerkleRoot(bytes32[] memory proof, uint256 path, bytes32 item) public returns (bytes32) {
    return root;
  }
}

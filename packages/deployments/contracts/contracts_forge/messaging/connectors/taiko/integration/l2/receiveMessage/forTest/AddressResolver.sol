...
abstract contract AddressResolver {
  ...
  modifier onlyFromNamed2(bytes32 name1, bytes32 name2) {
    if (msg.sender != resolve(name1, true) && msg.sender != resolve(name2, true)) revert RESOLVER_DENIED();
    _;
  }
  ...
  /// @return addr Address associated with the given name on the specified
  /// chain.
  function _resolve(uint256 chainId, bytes32 name, bool allowZeroAddress) private view returns (address payable addr) {

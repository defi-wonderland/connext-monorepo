...
interface IRecallableMessageSender {
  function onMessageRecalled(IBridge.Message calldata message) external payable;
}
...
interface IBridge {
  ...
  /// the message.
  function retryMessage(Message calldata message, bool isLastAttempt) external;
  ...
  function recallMessage(IBridge.Message calldata message, bytes calldata proof) external;
